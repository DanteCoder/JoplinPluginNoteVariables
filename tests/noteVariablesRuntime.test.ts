...
const code = fs.readFileSync(
  path.resolve(__dirname, '../src/noteVariablesRuntime.js'),
  'utf-8',
);

function getRuntime() {
  return (window as unknown as Record<string, unknown>).__noteVariablesRuntime as Record<string, unknown>;
}

beforeAll(() => {
  (globalThis as Record<string, unknown>).webviewApi = {
    postMessage: vi.fn(),
  };
  eval(code);
});
...
describe('escapeHtml', () => {
  const escapeHtml = () => getRuntime().escapeHtml as (s: string) => string;
...
describe('mergeGroupVariables', () => {
  const mergeGroupVariables = () => getRuntime().mergeGroupVariables as (
...
describe('replaceVariableKeysInTextNodes', () => {
  const replaceVariableKeysInTextNodes = () =>
    getRuntime().replaceVariableKeysInTextNodes as (
      nodes: Text[],
...
describe('getImportNamesFromMarker', () => {
  const getImportNamesFromMarker = () =>
    getRuntime().getImportNamesFromMarker as (el: Element) => string[];
...
describe('isInSubstitutionScope', () => {
  const isInSubstitutionScope = () =>
    getRuntime().isInSubstitutionScope as (node: Node) => boolean;
...
describe('renderImportMarkerWithStatus', () => {
  const renderImportMarkerWithStatus = () =>
    getRuntime().renderImportMarkerWithStatus as (
      marker: Element,
      importNames: string[],
      variableGroups: Record<string, { vars: Record<string, string> }>,
    ) => void;
...
describe('findUnprocessedImportMarkers', () => {
  const findUnprocessedImportMarkers = () =>
    getRuntime().findUnprocessedImportMarkers as () => {
      markers: Element[];
...
describe('collectEligibleTextNodes', () => {
  const collectEligibleTextNodes = () =>
    getRuntime().collectEligibleTextNodes as (root: Node) => Text[];
...
describe('fetchVariableGroupsFromPlugin', () => {
  const fetchVariableGroupsFromPlugin = () =>
    getRuntime().fetchVariableGroupsFromPlugin as (
      id: string,
    ) => Promise<Record<string, unknown> | null>;

  it('returns parsed groups from webviewApi', async () => {
    const mockPostMessage = (globalThis as Record<string, unknown>).webviewApi as {
      postMessage: ReturnType<typeof vi.fn>;
    };
    mockPostMessage.postMessage.mockResolvedValue({
      '%A%': { vars: { x: '1' } },
    });

    const result = await fetchVariableGroupsFromPlugin()('scriptId');
    expect(result).toEqual({ '%A%': { vars: { x: '1' } } });
    expect(mockPostMessage.postMessage).toHaveBeenCalledWith('scriptId', {
  ...
  it('returns null when webviewApi throws', async () => {
    ...
    mockPostMessage.postMessage.mockRejectedValue(new Error('fail'));
