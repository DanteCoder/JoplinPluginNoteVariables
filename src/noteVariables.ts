...
export namespace noteVariables {
  ...
  export async function init() {
    ...
    await dialogs.setButtons(handle_bad_vars_note, [{ id: 'Overwrite' }, { id: 'Do nothing' }]);
  ...
  async function syncData(sync_type: string) {
    ...
    if (!noteIntegrity.integrity) {
      ...
      if (result.id === 'Overwrite') {
        await joplin.data.put(['notes', noteIntegrity.noteId], null, {
          body: JSON.stringify(localVariables, null, '\t'),
        });
      } else {
        return;
      }
